...
async function completeDelivery(env, b) {
  ...
  // ส่งสถานะกลับ TRCloud (งานที่ดึงมาจาก SO) — ไม่บล็อกถ้า TRC ล้ม
  let trcloudPush = null;
  if (b.deliveryId) {
    try {
      const trcloud = require('./trcloud');
      trcloudPush = await trcloud.pushDeliveryCompleted(env, b.deliveryId);
      if (trcloudPush && trcloudPush.ok) {
        await env.DB.prepare('insert into activity_logs (action, reference_id, description, user_name, timestamp) values (?1,?2,?3,?4,?5)')
          .bind('TRCLOUD_DELIVERED', b.deliveryId, 'อัปเดต TRCloud ส่งสำเร็จ ' + (trcloudPush.invoiceNo || ''), b.user || '', new Date().toISOString()).run().catch(() => {});
      }
    } catch (e) {
      trcloudPush = { ok: false, error: String((e && e.message) || e) };
    }
  }
  return { ok: true, trcloud: trcloudPush };
}
