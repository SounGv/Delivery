...
/** ค้นหาบิลย้อนหลังข้ามวัน — เลขบิล / PO / ชื่อร้าน / รหัสงาน */
async function searchDeliveries(env, p = {}) {
  const q = String(p.q || p.query || '').trim();
  if (!q || q.length < 2) return [];
  const limit = Math.min(Math.max(Number(p.limit) || 40, 1), 80);
  const like = '%' + q.replace(/%/g, '') + '%';
  const { results } = await env.DB.prepare(
    `select * from deliveries
     where is_deleted = 0
       and (
         lower(coalesce(invoice_no,'')) like lower(?1)
         or lower(coalesce(po_no,'')) like lower(?1)
         or lower(coalesce(customer_name,'')) like lower(?1)
         or lower(coalesce(delivery_id,'')) like lower(?1)
         or lower(coalesce(note,'')) like lower(?1)
       )
     order by
       case when lower(coalesce(invoice_no,'')) = lower(?2) then 0
            when lower(coalesce(po_no,'')) = lower(?2) then 1
            else 2 end,
       delivery_date desc, updated_at desc
     limit ?3`
  ).bind(like, q, limit).all();
  return toFrontendList('deliveries', results || []);
}
...
module.exports = {
  ...
  getBootstrap, getDashboardData, getDeliveries, searchDeliveries, getRoutes, getRouteStops, getRouteGpsTrack,
