...
async function getDashboardData(env, p) {
  ...
  const [dels, routes, expenses, vehicles] = await Promise.all([
    DB.prepare('select * from deliveries where delivery_date = ?1 and is_deleted = 0').bind(date).all().then((r) => r.results),
  ...
  const d = dels || [], r = routes || [], e = expenses || [], v = vehicles || [];
...
async function getBootstrap(env, p) {
  const DB = env.DB;
  const date = (p && p.date) || todayStr();
  const [settings, customers, employees, vehicles, providers, extVehicles, dashboard, deliveries, routes, cartrack] = await Promise.all([
    ...
    getDashboardData(env, { date }),
    DB.prepare('select * from deliveries where delivery_date = ?1 and is_deleted = 0').bind(date).all().then((r) => toFrontendList('deliveries', r.results)),
  ...
  return {
    ...
    externalProviders: providers, externalVehicles: extVehicles, dashboard, deliveries, routes, cartrack,
  };
}
...
async function getRouteGpsTrack(env, p) {
  ...
  const { results } = await env.DB.prepare('select * from gps_logs where route_id = ?1 order by timestamp asc').bind(p.routeId).all();
  return toFrontendList('gps_logs', results);
}
...
async function getReports(env, p) {
  ...
  const build = (base, dateCol, extra) => {
    ...
    const sql = base + (conds.length ? ' where ' + conds.join(' and ') : '');
  ...
  const [dels, routes, expenses] = await Promise.all([
  ...
  return {
    ...
    routes: toFrontendList('routes', routes),
    expenses: toFrontendList('expenses', expenses),
  };
}
...
module.exports = {
  todayStr, getBootstrap, getDashboardData, getDeliveries, getRoutes, getRouteStops, getRouteGpsTrack,
