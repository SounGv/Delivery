...
/** Stable marker written to TRCloud invoice_note + local delivery note when delivered */
const DISPATCH_DELIVERED_MARKER = '[DISPATCH:DELIVERED]';
const DISPATCH_DELIVERED_NOTE_RE = /\[DISPATCH:DELIVERED\]|Dispatch\s*ส่งสำเร็จ/i;

function dispatchDeliveredStamp(iso) {
  return (iso || new Date().toISOString()).slice(0, 16).replace('T', ' ');
}

function dispatchDeliveredMarker(stamp) {
  return DISPATCH_DELIVERED_MARKER + ' Dispatch ส่งสำเร็จ ' + dispatchDeliveredStamp(stamp);
}

/** True when TRCloud SO/IV already shows Dispatch delivered (note marker or status_deliver full) */
function isTrcloudDelivered(docOrHead) {
  const h = soHead(docOrHead) || docOrHead || {};
  const note = String(blank(h.invoice_note) + ' ' + blank(h.note));
  if (DISPATCH_DELIVERED_NOTE_RE.test(note)) return { reason: 'dispatch_marker' };
  const qty = Number(h.sum_quantity);
  const delivered = Number(h.status_deliver);
  if (Number.isFinite(qty) && qty > 0 && Number.isFinite(delivered) && delivered >= qty) {
    return { reason: 'status_deliver_full' };
  }
  return null;
}

/** If TRCloud says delivered → mark Completed and keep original due/doc date (not today's queue) */
function applyTrcloudDeliveredPolicy(data, docOrHead) {
  const hit = isTrcloudDelivered(docOrHead);
  if (!hit) return data;
  data.Status = 'Completed';
  data._trcloudDeliveredReason = hit.reason;
  const anchor = String(data.DueDate || data.DocumentDate || data.DeliveryDate || '').slice(0, 10);
  if (anchor) data.DeliveryDate = anchor;
  return data;
}

function noteHasDispatchDelivered(note) {
  return DISPATCH_DELIVERED_NOTE_RE.test(String(note || ''));
}

async function appendLocalDeliveredNote(env, deliveryId, marker) {
  if (!deliveryId || !marker) return;
  const row = await env.DB.prepare('select note from deliveries where delivery_id = ?1 and is_deleted = 0')
    .bind(deliveryId).first();
  if (!row) return;
  const note = String(row.note || '');
  if (noteHasDispatchDelivered(note)) return;
  const newNote = [note, marker].filter(Boolean).join(' · ').slice(0, 500);
  await env.DB.prepare('update deliveries set note = ?1, updated_at = ?2 where delivery_id = ?3')
    .bind(newNote, new Date().toISOString(), deliveryId).run();
}
...
async function markSoDelivered(env, deliveryRow) {
  ...
  const stamp = dispatchDeliveredStamp();
  const marker = dispatchDeliveredMarker(stamp);
...
async function pushDeliveryCompleted(env, deliveryId) {
  ...
  const marker = dispatchDeliveredMarker();
  await appendLocalDeliveredNote(env, deliveryId, marker).catch(() => {});
  const inv = String(row.invoice_no || '');
  const note = String(row.note || '');
  const looksTrc = /^(KSO|BSO)/i.test(inv) || /TRCloud|TRC#/i.test(note);
  if (!looksTrc) return { ok: false, skipped: true, reason: 'not_trcloud_order', invoiceNo: inv, marker };
  const result = await markSoDelivered(env, row);
  if (result && result.ok && result.note) {
    await appendLocalDeliveredNote(env, deliveryId, result.note).catch(() => {});
  }
  return result;
}
...
async function upsertDeliveryFromSo(env, doc, { user, skipGeocode, workDate } = {}) {
  ...
  applyTrcloudDeliveredPolicy(data, doc);
...
function mapSoToDeliveryLight(doc) {
  const data = mapSoToDelivery(doc);
  const h = soHead(doc) || {};
  const staff = blank(h.salesman || h.staff);
...
/** ข้อมูลจาก so/search ไม่พอสร้างงานส่ง — ข้ามไม่ให้แถวว่างค้างในคิว */
function isIncompleteTrcloudSyncData(data) {
  const amt = Number(data.Amount);
  if (Number.isFinite(amt) && amt > 0) return false;
  const qty = Number(data._pieceQty || data.BoxQty);
  const addr = String(data.Address || '').trim();
  if (qty > 0 && addr.length > 10) return false;
  return true;
}

function normCustomerKey(name) {
...
async function syncOrders(env, p = {}) {
  ...
  const summary = {
    imported: 0, updated: 0, skipped: 0, deduped: 0, markedDelivered: 0, skippedDelivered: 0,
    skippedIncomplete: 0, ghostsPurged: 0,
    errors: [], total: 0,
  ...
  for (const item of allDocs) {
    try {
      ...
      if (isIncompleteTrcloudSyncData(data)) {
        summary.skippedIncomplete++;
        item.fmtSummary.skipped++;
        summary.skipped++;
        continue;
      }
      applyTrcloudDeliveredPolicy(data, item.row);
      applyWorkDatePolicy(data, workDate, lookback);
      if (data.Status === 'Completed' && data._trcloudDeliveredReason) summary.markedDelivered++;
  ...
  for (const item of mapped) {
    ...
    if (existing && locked) {
      summary.skipped++;
      if (existing.status === 'Completed') summary.skippedDelivered++;
      item.fmtSummary.skipped++;
  ...
  const customersSynced = new Set(
    mapped.map((item) => normCustomerKey(item.data.CustomerName)).filter(Boolean)
  );
  if (customersSynced.size) {
    const ghostPurgeStmts = [];
    for (const ck of customersSynced) {
      ghostPurgeStmts.push(env.DB.prepare(
        `update deliveries set is_deleted = 1, updated_at = ?1, updated_by = 'ghost-purge-sync',
         version = coalesce(version,0) + 1
         where is_deleted = 0
           and lower(trim(customer_name)) = ?2
           and (invoice_no is null or trim(invoice_no) = '')
           and (amount is null or amount <= 0)
           and (po_no is null or trim(po_no) = '')
           and (route_id is null or route_id = '')
           and status in ('Draft','Pending','')`
      ).bind(now, ck));
    }
    for (let i = 0; i < ghostPurgeStmts.length; i += 40) {
      await env.DB.batch(ghostPurgeStmts.slice(i, i + 40));
    }
    summary.ghostsPurged = ghostPurgeStmts.length;
  }
...
function soLinesForPrint(doc) {
  return soLines(doc).map((p) => ({
    sku: blank(p.product_id || p.sku || p.code),
    name: blank(p.description || p.product || p.product_name || p.product_id) || 'สินค้า',
    qty: Number(p.quantity) || 0,
    unit: blank(p.unit) || 'ชิ้น',
    price: Number(p.price) || 0,
    total: Number(p.total) || 0,
  }));
}

async function getSaleOrderPrint(env, p) {
  const id = String((p && (p.id || p.deliveryId)) || '').trim();
  if (!id) throw new Error('ต้องระบุบิล');
  const row = await env.DB.prepare('select * from deliveries where delivery_id = ?1 and is_deleted = 0').bind(id).first();
  if (!row) throw new Error('ไม่พบบิล ' + id);
  const delivery = toFrontend('deliveries', row);
  let lines = [];
  let salesman = '';
  let soId = '';
  if (configured(env)) {
    try {
      soId = await resolveSoId(env, row);
      if (soId) {
        const doc = await readOrder(env, soId);
        lines = soLinesForPrint(doc);
        salesman = blank((soHead(doc) || {}).salesman);
      }
    } catch (_) { /* header-only slip is still useful for น้าเอ๋ */ }
  }
  return { delivery, lines, salesman, soId };
}

async function getSaleOrdersPrint(env, p) {
  const ids = String((p && p.ids) || '')
    .split(/[,+\s]+/)
    .map((s) => s.trim())
    .filter(Boolean)
    .slice(0, 60);
  ...
  for (let i = 0; i < ids.length; i += conc) {
    ...
    const rows = await Promise.all(chunk.map(async (id) => {
      try { return await getSaleOrderPrint(env, { id }); }
      catch (e) { return { delivery: { DeliveryID: id }, lines: [], error: String((e && e.message) || e) }; }
    }));
    out.push(...rows);
  }
  return out;
...
module.exports = {
  ...
  DISPATCH_DELIVERED_MARKER, isTrcloudDelivered, noteHasDispatchDelivered, dispatchDeliveredMarker,
  getSaleOrderPrint, getSaleOrdersPrint,
};
