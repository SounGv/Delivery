...
/** ยกเลิกจัดรถ — คืนบิลเข้าคิววันนี้ เพื่อแก้รอบที่บันทึกผิด */
async function cancelRoute(env, b) {
  const DB = env.DB;
  const routeId = b.routeId || b.id;
  if (!routeId) throw new Error('ไม่ระบุรอบส่ง');
  const now = new Date().toISOString();
  await DB.batch([
    DB.prepare("update routes set status = ?1, updated_at = ?2 where route_id = ?3")
      .bind('Cancelled', now, routeId),
    DB.prepare("update deliveries set status = 'Draft', route_id = NULL, updated_at = ?1, version = coalesce(version,1) + 1 where route_id = ?2")
      .bind(now, routeId),
    logActivityStmt(DB, 'CANCEL_ROUTE', routeId, 'ยกเลิกจัดรถ คืนบิลเข้าคิววันนี้', b.user),
  ]);
...
module.exports = {
  ...
  createRoute, createExternalRoute, confirmRoute, updateRoute, updateRouteStop, cancelRoute,
