...
async function createDelivery(env, b) {
  ...
  const row = Object.assign(toDb('deliveries', b.data), {
    delivery_id: id, status: 'Draft', created_at: now, updated_at: now,
...
async function updateDelivery(env, b) {
  ...
  await autoGeocode(env, b.data);
  const cur = await env.DB.prepare('select version from deliveries where delivery_id = ?1').bind(b.id).first();
  return updateById(env.DB, 'deliveries', 'delivery_id', b.id, b.data, {
