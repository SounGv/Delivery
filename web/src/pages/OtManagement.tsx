...
export function OtManagement() {
  ...
  const allRecords = collectOtRecords(data.employees, otConfig, { start: monthStart, end: monthEnd })
  const records = employee === "all" ? allRecords : allRecords.filter((r) => r.employeeName === employee)

  const monthSummary = getOtSummary(records)
  const todaySummary = summaryForDate(data, data.todayDate, otConfig)
  const monthAllSummary = summaryForMonth(data, activeMonth, otConfig)

  const employeeNames = [...new Set(allRecords.map((r) => r.employeeName))].sort()

  // Layer the local approval decisions over the sheet-derived records.
  const effStatus = (r: OtRecord): OtApprovalStatus =>
    approvals[otRecordKey(r.date, r.employeeName)] ?? "PENDING"
  const pendingCount = records.filter((r) => effStatus(r) === "PENDING").length
  const approvedCount = records.filter((r) => effStatus(r) === "APPROVED").length
  ...
  return (
    <div className="space-y-4">
          ...
          <tbody>
            {records.map((r) => (
              <tr key={`${r.date}-${r.employeeName}`} className="border-b border-white/5 last:border-0">
                ...
                <td className="py-2 text-muted-foreground">{r.parcels ?? 0} พัสดุ · {r.items ?? 0} ชิ้น</td>
                <td className="py-2">
                  {(() => {
            ...
            {records.length === 0 && (
              <tr>
                <td colSpan={11} className="py-8 text-center text-muted-foreground">
                  {hasTime ? "ไม่มีรายการ OT ในเดือนนี้" : "ยังไม่มีข้อมูลเวลาเข้า-ออกงานในระบบ"}
                </td>
              </tr>
            )}
          </tbody>
          {records.length > 0 && (
            <tfoot>
              <tr className="border-t border-border font-semibold text-foreground">
                <td className="pt-2" colSpan={7}>รวม ({monthSummary.recordCount} รายการ)</td>
      ...
      <p className="px-1 text-[11px] text-muted-foreground">
        OT คำนวณจากเวลาออกงานจริงเทียบเวลาเลิกงานปกติ ({otConfig.workEndHour}:00) · นับเฉพาะชั่วโมงและงานที่ทำ ไม่คิดเป็นเงิน · เข้าก่อนเวลาไม่นับเป็น OT · กด "อนุมัติ" เพื่อรับรองรายการ — สถานะอนุมัติถูกบันทึกในเบราว์เซอร์นี้ (ชีทเป็นแบบอ่านอย่างเดียว จึงเขียนกลับไม่ได้)
      </p>
